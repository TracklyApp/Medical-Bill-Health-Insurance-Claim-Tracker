...
const cash=T.money, fmtDate=s=>s?new Date(s+'T12:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}):'—';
...
function reportsPage(){return heading('Medical expense report','All recorded dates · Generated '+fmtDate(T.today()),btn('Print / Save as PDF','print','','primary'))+'<p class="report-only">TracklyApp · '+(state.demo?'SAMPLE DATA · ':'')+'USD · Personal medical expense summary</p>'+stats()+panel('Bills & insurance',billRows(state.bills,'eob'))+panel('Payments, refunds & reimbursements',transactionTable(state.transactions))+panel('Appeal deadlines',deadlineList(T.deadlines(state).filter(e=>e.type==='Appeal deadline')))+'<p class="hint">All amounts in USD. Unknown EOB responsibility is excluded from Still Owed. You Paid is net of provider refunds; reimbursements are separate. This report summarizes your entered records.</p>';}
...
function amount(name,label,value,required=false){return field(name,label,value===null||value===undefined?'':(value/100).toFixed(2),'number','min="0" max="1000000000" step="0.01" '+(required?'required':''));}
